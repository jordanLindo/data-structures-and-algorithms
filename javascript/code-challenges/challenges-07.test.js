...
const sortStarWarsCharacters = (starWarsArr) => starWarsArr.sort((val1,val2)=> parseInt(val1.height) < parseInt(val2.height) ? 1 : parseInt(val1.height) > parseInt(val2.height) ? -1 : 0 );
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const listFoods = (recipe) => {
  ...
  for (let index = 0; index < len; index++) {
    let chunk = recipe.ingredients[index].slice(recipe.ingredients[index].indexOf(' ')+1);
...
const splitFoods = (recipe) => {
  ...
  let len = recipe.ingredients.length;
  for (let index = 0; index < len; index++) {
    result.push(recipe.ingredients[index].split(' ').slice(2).join(' '));
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => str.length <= numberOfCharacters ? '': numberOfCharacters > 0 ? str.slice(0,(str.length)-numberOfCharacters) : str;
...
const totalSumCSV = (str) => {
  ...
  // Solution code here...
  let arr = str.split(',');
  arr.forEach((val) => total+=parseInt(val));
  return total;
};
...
const removeVowels = (str) => str.split('').filter((val) => /[aeiou]/.test(val) ? false : true).join('');
...
const extractVowels = (str) => {
  ...
  let cons = [];
  let vow = [];
  str.split('').forEach((val) => /[aeiou]/.test(val) ? vow.push(val) : cons.push(val));
  vow.sort();
  let arr = [];
  arr.push(cons.join(''));
  arr.push(vow.join(''));
